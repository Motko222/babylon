...
pubkey=$(babylond tendermint show-validator --chain-id $NETWORK --log_format json | jq -r .key)
...
balance=$(babylond query bank balances $wallet --chain-id $NETWORK | grep amount | awk '{print $3}' | sed 's/"//g' | awk '{print $1 / 1000000}' )
